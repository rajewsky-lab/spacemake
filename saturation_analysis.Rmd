...
```{r plot_distributions_of_observations, echo =F}
clrs = c('umis' = '#E69F00', 'pcr' = '#56B4E9', 'reads' = '#009E73')

plot_observations <- function(metric, val_le){
    dge_data %>%
        select(CELL_BARCODE, percentage, pcr, umis, reads) %>%
        gather('obs', 'val', umis, pcr, reads) %>%
        filter(obs == metric, val < val_le) %>%
...
plot_observations('umis', 500)
plot_observations('reads', 500)
plot_observations('pcr', 2.5)
```
