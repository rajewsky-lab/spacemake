...
# TODO: check the IDs for the plots grouped by tabs
# TODO: add the nucleotide quartiles for the non-meshed modes
# TODO: add legend to the entropy plots

# How to run, example:
# python /home/dleonpe/spacemake_project/repos/spacemake/spacemake/report/classes/qc_sequencing.py \
# --project-id fc_sts_76 --sample-id fc_sts_76_3  --run-modes fc_sts_novaseq_SP_mesh_7um \
# --dge-summary-paths /home/dleonpe/spacemake_project/data/1_spacemake_mouse_new/projects/fc_sts_76/processed_data/fc_sts_76_3/illumina/complete_data/dge/dge.all.polyA_adapter_trimmed.mm_included.spatial_beads.mesh_7_hexagon_fc_010_L3_tile_2157.obs.csv \
# --complete-data-root /home/dleonpe/spacemake_project/data/1_spacemake_mouse_new/projects/fc_sts_76/processed_data/fc_sts_76_3/illumina/complete_data \
# --split-reads-read-type /home/dleonpe/spacemake_project/data/1_spacemake_mouse_new/projects/fc_sts_76/processed_data/fc_sts_76_3/illumina/complete_data/split_reads.polyA_adapter_trimmed/read_type_num.txt \
# --is-spatial True --out-html-report /home/dleonpe/spacemake_project/repos/spacemake/spacemake/report/classes/fc_sts_76_3_qc_sequencing_new.html \
# --puck-barcode-file-id-qc fc_010_L3_tile_2157
...
def plot_umi_cutoff(obs_df):
    ...
    def summarise_dge_summary(df, umi_cutoff):
        ...
        df_filter["n_beads"] = len(df_filter)
    ...
    umi_cutoff_data = pd.DataFrame(
        np.vstack(
            [
                summarise_dge_summary(obs_df, umi_cutoff).values
...
def generate_qc_sequencing_metadata(
    project_id: str,
    sample_id: str,
    dge_summary_paths: Union[
        list, dict
    ],  # from snakemake_helper_funtions.get_qc_sheet_input_files
    ...
    if (
        ...
        dge_summary_paths = {
            run_mode: dge_summary_path
            for run_mode, dge_summary_path in zip(run_modes, dge_summary_paths)
        }
    elif (
        isinstance(run_modes, list)
        and isinstance(dge_summary_paths, list)
        ...
        raise ValueError(
            "'run_modes' and 'dge_summary_paths' must have the same length"
        )
    ...
    for s in split_reads_read_type:
        ...
        mappingstat = {'name': os.path.basename(os.path.dirname(s)), 'all_stats': generate_table_mapping_statistics(complete_data_root, s)}
    ...
    for run_mode in run_modes:
        ...
        _dge_summary = load_dge_summary(dge_summary_paths[run_mode])
        dge_summaries[run_mode]["df"] = _dge_summary[0]
        dge_summaries[run_mode]["adata"] = _dge_summary[1]

        obs_df = dge_summaries[run_mode]["df"]
        metrics_beads = (
            obs_df[["n_genes_by_counts", "reads_per_counts", "n_reads", "total_counts"]]
            .median()
            .to_dict()
        )
    ...
    for run_mode, dge_summary in dge_summaries.items():
        ...
        axis.plot(
            np.arange(len(obs_df)), np.cumsum(obs_df["n_reads"].values), color="black", linewidth=1
        )
...
def cmdline():
    ...
    # TODO: implement dge_summary_paths
