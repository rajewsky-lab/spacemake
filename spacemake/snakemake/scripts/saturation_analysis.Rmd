...
plot_observations <- function(metric, run_mode_in, log_scale = T){
    pl <- dge_data %>%
...
plot_deciled_data <- function(run_mode_in) {
    ...
    pl <- decile_dat %>%
        ggplot(aes(percentage, value, color= decile, fill = decile)) +
            ...
            facet_wrap(~observation, scales = 'free_y', ncol=3) +
...
plot_data <- function(run_mode_in, umi_cutoff=c(1, 100, 200)){
    ...
    ggplot(aes(percentage, value, color = observation, fill=observation, linetype=umi_cutoff)) +
        ...
        guides(colour = guide_legend(nrow=2, byrow=T, override.aes = list(size=3)))
}
```


## Plots per run\_mode {.tabset}

```{r plot_distributions_of_observations, echo =F, fig.width=12,fig.height=7,results='asis'}
...
for (run_mode_in in run_mode_names){
    ...
    cat(paste0('\n\n### ', run_mode_in, '\n\n'))
    print(plot_observations('umis', run_mode_in))
    print(plot_observations('reads', run_mode_in))
    print(plot_observations('pcr', run_mode_in, log_scale=F))
