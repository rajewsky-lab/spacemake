...
#write_rds(snakemake, '/data/rajewsky/home/tsztank/projects/spatial/repos/sts-paper/seq_scope_saturation_analysis.rds')
```
...
plot_data <- function(run_mode_in, umi_cutoff=c(1, 100, 200)){
    ...
    ggplot(aes(percentage, value, color = observation, fill=observation, linetype=umi_cutoff)) +
        ...
        scale_x_continuous(breaks=seq(0, 100, 20)) +
        facet_wrap(~observation, scales = 'free', ncol=2) +
        labs(y='', x='downsampling percentage', color='', fill='', linetype='') +
        theme(strip.background=element_blank(), legend.position='bottom',
              legend.key.width=unit(1.2, 'cm')) +
        guides(colour = guide_legend(nrow=3, byrow=T, override.aes = list(size=3)),
               linetype = guide_legend(nrow=3, byrow=T, override.aes = list(size=2)))
}
```
...
for (run_mode_in in run_mode_names){
    ...
    cat(paste0('\n\n### ', run_mode_in, '\n\n'))
    print(plot_observations('umis', run_mode_in, ttl='# of UMIs per spot'))
    print(plot_observations('reads', run_mode_in, ttl='# of reads per spot'))
    print(plot_observations('pcr', run_mode_in, log_scale=F, ttl='reads / UMI per spot'))
...
```{r plot_median_values_of, echo =F, fig.width=7,fig.height=7,results='asis'}
run_mode_names <- dge_data %$% 
    run_mode %>% unique()
...
for (run_mode_in in run_mode_names){
    ...
    print(plot_data(run_mode_in, umi_cutoff = c(1, umi_cutoff)))
}
```
