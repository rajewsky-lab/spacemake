...
#snakemake <- read_rds('slideseq_qc.rds')
...
```{r plot_n_reads_bead_hist, echo = F, fig.width=12.5, fig.height=5, dpi=200, results='asis'}
to_plot <- dge_summary %>%
...
metric_plot <- function(run_mode_in, metric, legend_pos='none'){
    ...
    pl <-  to_plot %>%
       ...
       ggplot(aes(x = val, fill=obs)) +
            ...
            labs(fill='', x=paste0('# of ', y_label), y='# of spatial\nunits') +
