...
#snakemake <- read_rds('/data/rajewsky/home/tsztank/projects/spatial/repos/sts-paper/slideseq_qc.rds')
#write_rds(snakemake, '/data/rajewsky/home/tsztank/projects/spatial/repos/sts-paper/seq_scope_qc.rds')
```
...
```{r knee_plot, echo = F, fig.height=5}
...
read_counts %>%
    ggplot(aes(ix, reads_cumsum, linetype=run_mode)) +
        geom_line() +
...
metric_names <- list('umis' = 'UMIs', 'genes' = 'genes', 'pcr' = 'UMIs/reads',
                     'reads'='reads')
...
run_mode_info <- dge_summary %>%
    select(run_mode) %>%
    summarise(n_cells=n()) %>%
    mutate(def_plot_bead_size = ifelse(n_cells > 5000, 0.5, 1)) %>%
...
plot_spatial_qc <- function(variable, dat_in, plot_bead_size=0.1, ttl='', cut_dist=T){
...
```{r plot_per_run_mode, echo =F, results='asis', fig.width=7, fig.height=6, eval=is_spatial}
for (run_mode_name in run_mode_names){
    ...
    for (qc_var in qc_var_info$qc_vars){
        ...
        print(plot_spatial_qc(
                  qc_var, dat_in, plot_bead_size = pbs,
