...
#write_rds(snakemake, '/data/rajewsky/home/tsztank/projects/spatial/repos/sts-paper/seq_scope_automated_analysis_2.rds')
#snakemake <- read_rds('seq_scope_automated_analysis_2.rds')
source(snakemake@params$r_shared_scripts)
```

```{r read_data, echo = F}
obs_df <- read_table(snakemake@input$obs_df) %>%
    mutate(pct_counts_mt = as.double(pct_counts_mt))
var_df <- read_table(snakemake@input$var_df)
...
if (is_spatial){
    ...
    puck_bead_size <- max(def_plot_bead_size, ifelse(snakemake@params$run_mode_variables$mesh_data,
        snakemake@params$run_mode_variables$mesh_spot_diameter_um / 50,
        snakemake@params$puck_variables$spot_diameter_um / 50))
...
```{r nhood_enrichment_title, results='asis', eval=is_spatial}
cat('\n\n### Neighborhood enrichment\n\n')
```

```{r plot_nhood_enrichment, echo =F, eval=is_spatial}
nhood_dat <- read_table(snakemake@input$nhood_enrichment)
