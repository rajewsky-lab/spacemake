...
with gzip.open(snakemake.input[0], 'rt') as fastq_in:
    ...
    for read in fastq_in:
        if line == 1:
            ...
            kmer_hashes = ['_'.join(prod) for prod in itertools.product(kmers, [str(x) for x in position_list])]
            position_counts = pd.DataFrame(0, index = kmer_hashes, columns = ['count'])
        
        # if line is a read
        if line % 4 == 1:
            kmer_hashes = kmer_hashes + [str(read[i:i+kmer_len]) + '_' + str(i) for i in position_list]
        ...
        if line % 4000 == 0:
            ...
            # update df
            position_counts.loc[kmer_hash_counts.keys(), 'count'] = position_counts.loc[kmer_hash_counts.keys(), 'count'] + list(kmer_hash_counts.values())
