...
read_metrics <- metadata %>%
    ...
    mutate(star_log = paste0('/data/rajewsky/projects/slide_seq/projects/', project_id, '/processed_data/', sample_id, '/illumina/complete_data/star_Log.final.out'),
           read_types =paste0('/data/rajewsky/projects/slide_seq/projects/', project_id, '/processed_data/', sample_id, '/illumina/complete_data/uniquely_mapped_reads_type.txt')) %>%
...
strand_info <- metadata %>%
    select(project_id, sample_id, puck_id, species, sequencing_date) %>%
    mutate(filename = paste0('/data/rajewsky/projects/slide_seq/projects/', project_id, '/processed_data/', sample_id, '/illumina/complete_data/strand_info.txt'),
...
read_dge_summary <- function(filename){
    tibble(umi_cutoff = umi_cutoffs, filename=filename) %>%
        ...
        summarise(
                ...
                mean_umi = mean(NUM_TRANSCRIPTS),
                mean_reads = mean(NUM_GENIC_READS),
                mean_genes = mean(NUM_GENES),
                num_beads = n())
...
barcode_metadata <- metadata %>%
    select(project_id, sample_id, puck_id, species, sequencing_date) %>%
    mutate(filename = paste0('/data/rajewsky/projects/slide_seq/projects/', project_id, '/processed_data/', sample_id, '/illumina/complete_data/dge/dge_all_summary.txt'),
