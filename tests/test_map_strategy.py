...
def test_validation_cmdline_issue_54(configured_root):
    ...
    data = [
        ...
        ("with_cflavor", "rRNA:bowtie2@custom_index->genome:STAR@default", 'test_hsa', True),
        ("unknown_cflavor", "rRNA:bowtie2@customX->genome:STAR@defaultBLA", 'test_hsa', False),
        # ("flipped", "bowtie2:rRNA->STAR:genome", "rRNA:bowtie2->genome:STAR"),
    ]
    for name, mapstr, species, expect_pass in data:
        ...
        sm(
            "projects", "add_sample",
            "--project_id=test", f"--sample_id={name}",
